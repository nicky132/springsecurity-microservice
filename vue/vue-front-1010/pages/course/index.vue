...
export default {
  ...
  methods:{
    ...
    search(pId,index){ //点击一级分类 查询对应二级分类
      //为了样式生效
      this.oneIndex = index;
      this.twoIndex = -1;
      this.searchObj = {};
      ...
      for(let i=0;i<this.subjectNestedList.length;i++){
        if(this.subjectNestedList[i].id==pId){
          this.subSubjectList = this.subjectNestedList[i].children;
          break;
        }
      }
      // 根据一级分类 条件查询 显示课程
      this.searchObj.subjectParentId = pId;
      this.gotoPage(1);
    },
    searchTwo(id,index){ //点击二级分类 进行查询
        //为了样式生效
        this.twoIndex = index;
